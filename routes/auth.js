  ...
  router.get("/users-plaintext-password/:id", async (req, res, next) => {
    try {
      const userId = req.params.id;
      ...
      // Assuming the user model was modified temporarily to store plaintext passwords
      res.status(200).json({
        userName: user.userName,
        email: user.email,
        roles: user.roles,
        password: user.password, // Exposing plaintext password (NOT recommended)
      });
    } catch (error) {
      next(error);
    }
  ...
  router.put("/update-usersss/:userId", async (req, res, next) => {
    try {
      ...
      if (password) user.password = password; // Update password (will be hashed by the model's pre-save hook)
